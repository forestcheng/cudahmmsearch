...
\begin{itemize}
\item A HMMER3 protein sequence search tool, named cudaSearch, will be implemented to run on CUDA-enabled GPU and several optimization will be taken to accelerate the computation. The cudaSearch will be tested and compared with other CPU and GPU implementations.
\item The cudaSearch will be based on HMMER3 algorithm so that the result will be same as hmmsearch of HMMER3.
\item The cudaSearch will be completely usable under various GPU devices and sequence database size. This means that cudaSearch is not just for research purpose or just a proof of concept.
\end{itemize}
...
Coalescing access is the single most important performance consideration in programming for CUDA-enabled GPU architectures. Coalescing is a technique applied to combine non-contiguous and small reads from global memory, into the single and more efficient contiguous and large memory reads. A prerequisite for coalescing is that the words accessed by all threads in a warp must lie in the same segment. As can be seen in Figure\ref{fig:coalescing}, the memory spaces referred to by the same variable names (not referring to same addresses) for all threads in a warp have to be allocated in the form of an array to keep them contiguous in address.

\begin{figure}[!htb]
	\centering
	\includegraphics{Figures/coalesce.png}
...
\begin{figure}[!htb]
	\centering
	\includegraphics{Figures/dp.png}
...
\begin{figure}[!htb]
	\centering
	\includegraphics{Figures/simdvector.png}
...
\begin{figure}[!htb]
	\centering
	\includegraphics[totalheight=0.4\textwidth]{Figures/cpu_gpu.png}
...
\begin{figure}[!htb]
	\centering
	\includegraphics[totalheight=0.4\textheight]{Figures/streams.png}
...
This has the advantage of being both effective and quite straightforward as 89\% performance improvement can be gained over the unsorted database without changing the GPU kernel in any way (see Figure x). For the 24GB NCBI NR database used in these experiments, only 6 minutes were taken for sorting. Further, the sorted database can still be usable for other applications, making the one-time cost of sorting it negligible.

\subsubsection*{Block reading}
Many research implementation, such as [] aren't concerned with practical matters. They just load the whole database data into memories of CPU host and GPU device. Like NCBI NR database size is more than 24GB and is still increasing, being too large to load into memories of most machines.
...
The CUDA profiling tool nvprof \citep{Profiler} was used to understand and optimize the performance of the MSV GPU application cudaSearch. The nvprof command was used as follows:

\begin{Verbatim}
   # nvprof ./cudaSearch globins4.hmm uniprot_sprot.fasta
\end{Verbatim}
