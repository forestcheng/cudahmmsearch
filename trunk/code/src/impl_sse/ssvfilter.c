...
int8_t SaturateSubChar(int8_t x, int8_t y)
{
	short z = x - y;
	if (z > CHAR_MAX) return CHAR_MAX;
	if (z < CHAR_MIN) return CHAR_MIN;
	return z;
}
...
#define STEP_SINGLE16(svx16)                         \
  for (int j = 0; j < 16; ++j) {						\
	  svx16[j] = SaturateSubChar(svx16[j], *(rsc16 + j));	\
	  xEv16[j] = max(xEv16[j], svx16[j]);				\
  }		\
  rsc16 += 16;
...
#define STEP_BANDS_1x16()                          \
  STEP_SINGLE16(sv00x16)

#define STEP_BANDS_2()                          \
  STEP_BANDS_1()                                \
  STEP_SINGLE(sv01)

#define STEP_BANDS_2x16()                          \
  STEP_BANDS_1x16()                                \
  STEP_SINGLE16(sv01x16)

#define STEP_BANDS_3()                          \
  STEP_BANDS_2()                                \
  STEP_SINGLE(sv02)

#define STEP_BANDS_3x16()                          \
  STEP_BANDS_2x16()                                \
  STEP_SINGLE16(sv02x16)

#define STEP_BANDS_4()                          \
  STEP_BANDS_3()                                \
  STEP_SINGLE(sv03)

#define STEP_BANDS_4x16()                          \
  STEP_BANDS_3x16()                                \
  STEP_SINGLE16(sv03x16)

#define STEP_BANDS_5()                          \
  STEP_BANDS_4()                                \
  STEP_SINGLE(sv04)

#define STEP_BANDS_5x16()                          \
  STEP_BANDS_4x16()                                \
  STEP_SINGLE16(sv04x16)

#define STEP_BANDS_6()                          \
  STEP_BANDS_5()                                \
  STEP_SINGLE(sv05)

#define STEP_BANDS_6x16()                          \
  STEP_BANDS_5x16()                                \
  STEP_SINGLE16(sv05x16)

#define STEP_BANDS_7()                          \
  STEP_BANDS_6()                                \
  STEP_SINGLE(sv06)

#define STEP_BANDS_7x16()                          \
  STEP_BANDS_6x16()                                \
  STEP_SINGLE16(sv06x16)

#define STEP_BANDS_8()                          \
  STEP_BANDS_7()                                \
  STEP_SINGLE(sv07)

#define STEP_BANDS_8x16()                          \
  STEP_BANDS_7x16()                                \
  STEP_SINGLE16(sv07x16)

#define STEP_BANDS_9()                          \
  STEP_BANDS_8()                                \
  STEP_SINGLE(sv08)

#define STEP_BANDS_9x16()                          \
  STEP_BANDS_8x16()                                \
  STEP_SINGLE16(sv08x16)

#define STEP_BANDS_10()                         \
  STEP_BANDS_9()                                \
  STEP_SINGLE(sv09)

#define STEP_BANDS_10x16()                         \
  STEP_BANDS_9x16()                                \
  STEP_SINGLE16(sv09x16)

#define STEP_BANDS_11()                         \
  STEP_BANDS_10()                               \
  STEP_SINGLE(sv10)

#define STEP_BANDS_11x16()                         \
  STEP_BANDS_10x16()                               \
  STEP_SINGLE16(sv10x16)

#define STEP_BANDS_12()                         \
  STEP_BANDS_11()                               \
  STEP_SINGLE(sv11)

#define STEP_BANDS_12x16()                         \
  STEP_BANDS_11x16()                               \
  STEP_SINGLE16(sv11x16)

#define STEP_BANDS_13()                         \
  STEP_BANDS_12()                               \
  STEP_SINGLE(sv12)

#define STEP_BANDS_13x16()                         \
  STEP_BANDS_12x16()                               \
  STEP_SINGLE16(sv12x16)

#define STEP_BANDS_14()                         \
  STEP_BANDS_13()                               \
  STEP_SINGLE(sv13)

#define STEP_BANDS_14x16()                         \
  STEP_BANDS_13x16()                               \
  STEP_SINGLE16(sv13x16)
...
#define CONVERT_STEPx16(step, length_check, label, pos, svx16)        \
  length_check(label)                                           \
  rsc16 = sbv16[dsq[i]] + ((pos) << 4);		\
  step()                                                        \
  for (int j = 14; j >= 0; --j) {		\
	  svx16[j+1] = svx16[j];				\
  }										\
...
#define CONVERT_1x16(step, LENGTH_CHECK, label)            \
  CONVERT_STEPx16(step, LENGTH_CHECK, label, Q - 1, sv00x16)

#define CONVERT_2(step, LENGTH_CHECK, label)            \
  CONVERT_STEP(step, LENGTH_CHECK, label, sv01, Q - 2)  \
  CONVERT_1(step, LENGTH_CHECK, label)

#define CONVERT_2x16(step, LENGTH_CHECK, label)            \
  CONVERT_STEPx16(step, LENGTH_CHECK, label, Q - 2, sv01x16)  \
  CONVERT_1x16(step, LENGTH_CHECK, label)

#define CONVERT_3(step, LENGTH_CHECK, label)            \
  CONVERT_STEP(step, LENGTH_CHECK, label, sv02, Q - 3)  \
  CONVERT_2(step, LENGTH_CHECK, label)

#define CONVERT_3x16(step, LENGTH_CHECK, label)            \
  CONVERT_STEPx16(step, LENGTH_CHECK, label, Q - 3, sv02x16)  \
  CONVERT_2x16(step, LENGTH_CHECK, label)

#define CONVERT_4(step, LENGTH_CHECK, label)            \
  CONVERT_STEP(step, LENGTH_CHECK, label, sv03, Q - 4)  \
  CONVERT_3(step, LENGTH_CHECK, label)

#define CONVERT_4x16(step, LENGTH_CHECK, label)            \
  CONVERT_STEPx16(step, LENGTH_CHECK, label, Q - 4, sv03x16)  \
  CONVERT_3x16(step, LENGTH_CHECK, label)

#define CONVERT_5(step, LENGTH_CHECK, label)            \
  CONVERT_STEP(step, LENGTH_CHECK, label, sv04, Q - 5)  \
  CONVERT_4(step, LENGTH_CHECK, label)

#define CONVERT_5x16(step, LENGTH_CHECK, label)            \
  CONVERT_STEPx16(step, LENGTH_CHECK, label, Q - 5, sv04x16)  \
  CONVERT_4x16(step, LENGTH_CHECK, label)

#define CONVERT_6(step, LENGTH_CHECK, label)            \
  CONVERT_STEP(step, LENGTH_CHECK, label, sv05, Q - 6)  \
  CONVERT_5(step, LENGTH_CHECK, label)

#define CONVERT_6x16(step, LENGTH_CHECK, label)            \
  CONVERT_STEPx16(step, LENGTH_CHECK, label, Q - 6, sv05x16)  \
  CONVERT_5x16(step, LENGTH_CHECK, label)

#define CONVERT_7(step, LENGTH_CHECK, label)            \
  CONVERT_STEP(step, LENGTH_CHECK, label, sv06, Q - 7)  \
  CONVERT_6(step, LENGTH_CHECK, label)

#define CONVERT_7x16(step, LENGTH_CHECK, label)            \
  CONVERT_STEPx16(step, LENGTH_CHECK, label, Q - 7, sv06x16)  \
  CONVERT_6x16(step, LENGTH_CHECK, label)

#define CONVERT_8(step, LENGTH_CHECK, label)            \
  CONVERT_STEP(step, LENGTH_CHECK, label, sv07, Q - 8)  \
  CONVERT_7(step, LENGTH_CHECK, label)

#define CONVERT_8x16(step, LENGTH_CHECK, label)            \
  CONVERT_STEPx16(step, LENGTH_CHECK, label, Q - 8, sv07x16)  \
  CONVERT_7x16(step, LENGTH_CHECK, label)

#define CONVERT_9(step, LENGTH_CHECK, label)            \
  CONVERT_STEP(step, LENGTH_CHECK, label, sv08, Q - 9)  \
  CONVERT_8(step, LENGTH_CHECK, label)

#define CONVERT_9x16(step, LENGTH_CHECK, label)            \
  CONVERT_STEPx16(step, LENGTH_CHECK, label, Q - 9, sv08x16)  \
  CONVERT_8x16(step, LENGTH_CHECK, label)

#define CONVERT_10(step, LENGTH_CHECK, label)           \
  CONVERT_STEP(step, LENGTH_CHECK, label, sv09, Q - 10) \
  CONVERT_9(step, LENGTH_CHECK, label)

#define CONVERT_10x16(step, LENGTH_CHECK, label)           \
  CONVERT_STEPx16(step, LENGTH_CHECK, label, Q - 10, sv09x16) \
  CONVERT_9x16(step, LENGTH_CHECK, label)
...
#define CONVERT_11x16(step, LENGTH_CHECK, label)           \
  CONVERT_STEPx16(step, LENGTH_CHECK, label, Q - 11, sv10x16) \
  CONVERT_10x16(step, LENGTH_CHECK, label)
...
#define CONVERT_12x16(step, LENGTH_CHECK, label)           \
  CONVERT_STEPx16(step, LENGTH_CHECK, label, Q - 12, sv11x16) \
  CONVERT_11x16(step, LENGTH_CHECK, label)
...
#define CONVERT_13x16(step, LENGTH_CHECK, label)           \
  CONVERT_STEPx16(step, LENGTH_CHECK, label, Q - 13, sv12x16) \
  CONVERT_12x16(step, LENGTH_CHECK, label)
...
#define CONVERT_14x16(step, LENGTH_CHECK, label)           \
  CONVERT_STEPx16(step, LENGTH_CHECK, label, Q - 14, sv13x16) \
  CONVERT_13x16(step, LENGTH_CHECK, label)
...
#define CALCx16(reset, step, convert, width)       \
  int i;                                        \
  int i2;                                       \
  int Q        = p7O_NQB(om->M);                \
  uint8_t *rsc16;                               \
...
void
calc_band_1x16(const ESL_DSQ *dsq, int L, const P7_OPROFILE *om, int q, uint8_t *beginv16, uint8_t *xEv16, uint8_t **sbv16)
{
	CALCx16(RESET_1x16, STEP_BANDS_1x16, CONVERT_1x16, 1)
}
...
void
calc_band_2x16(const ESL_DSQ *dsq, int L, const P7_OPROFILE *om, int q, uint8_t *beginv16, uint8_t *xEv16, uint8_t **sbv16)
{
  CALCx16(RESET_2x16, STEP_BANDS_2x16, CONVERT_2x16, 2)
}
...
void
calc_band_3x16(const ESL_DSQ *dsq, int L, const P7_OPROFILE *om, int q, uint8_t *beginv16, uint8_t *xEv16, uint8_t **sbv16)
{
  CALCx16(RESET_3x16, STEP_BANDS_3x16, CONVERT_3x16, 3)
}
...
void
calc_band_4x16(const ESL_DSQ *dsq, int L, const P7_OPROFILE *om, int q, uint8_t *beginv16, uint8_t *xEv16, uint8_t **sbv16)
{
  CALCx16(RESET_4x16, STEP_BANDS_4x16, CONVERT_4x16, 4)
}
...
void
calc_band_5x16(const ESL_DSQ *dsq, int L, const P7_OPROFILE *om, int q, uint8_t *beginv16, uint8_t *xEv16, uint8_t **sbv16)
{
  CALCx16(RESET_5x16, STEP_BANDS_5x16, CONVERT_5x16, 5)
}
...
void
calc_band_6x16(const ESL_DSQ *dsq, int L, const P7_OPROFILE *om, int q, uint8_t *beginv16, uint8_t *xEv16, uint8_t **sbv16)
{
  CALCx16(RESET_6x16, STEP_BANDS_6x16, CONVERT_6x16, 6)
}
...
void
calc_band_7x16(const ESL_DSQ *dsq, int L, const P7_OPROFILE *om, int q, uint8_t *beginv16, uint8_t *xEv16, uint8_t **sbv16)
{
  CALCx16(RESET_7x16, STEP_BANDS_7x16, CONVERT_7x16, 7)
}
...
void
calc_band_8x16(const ESL_DSQ *dsq, int L, const P7_OPROFILE *om, int q, uint8_t *beginv16, uint8_t *xEv16, uint8_t **sbv16)
{
  CALCx16(RESET_8x16, STEP_BANDS_8x16, CONVERT_8x16, 8)
}
...
void
calc_band_9x16(const ESL_DSQ *dsq, int L, const P7_OPROFILE *om, int q, uint8_t *beginv16, uint8_t *xEv16, uint8_t **sbv16)
{
  CALCx16(RESET_9x16, STEP_BANDS_9x16, CONVERT_9x16, 9)
}
...
void
calc_band_10x16(const ESL_DSQ *dsq, int L, const P7_OPROFILE *om, int q, uint8_t *beginv16, uint8_t *xEv16, uint8_t **sbv16)
{
  CALCx16(RESET_10x16, STEP_BANDS_10x16, CONVERT_10x16, 10)
}
...
void
calc_band_11x16(const ESL_DSQ *dsq, int L, const P7_OPROFILE *om, int q, uint8_t *beginv16, uint8_t *xEv16, uint8_t **sbv16)
{
  CALCx16(RESET_11x16, STEP_BANDS_11x16, CONVERT_11x16, 11)
}
...
void
calc_band_12x16(const ESL_DSQ *dsq, int L, const P7_OPROFILE *om, int q, uint8_t *beginv16, uint8_t *xEv16, uint8_t **sbv16)
{
  CALCx16(RESET_12x16, STEP_BANDS_12x16, CONVERT_12x16, 12)
}
...
void
calc_band_13x16(const ESL_DSQ *dsq, int L, const P7_OPROFILE *om, int q, uint8_t *beginv16, uint8_t *xEv16, uint8_t **sbv16)
{
  CALCx16(RESET_13x16, STEP_BANDS_13x16, CONVERT_13x16, 13)
}
...
void
calc_band_14x16(const ESL_DSQ *dsq, int L, const P7_OPROFILE *om, int q, uint8_t *beginv16, uint8_t *xEv16, uint8_t **sbv16)
{
  CALCx16(RESET_14x16, STEP_BANDS_14x16, CONVERT_14x16, 14)
}
#endif /* MAX_BANDS > 6 */
#if MAX_BANDS > 14
__m128i
...
uint8_t
get_xE_RegDBG(const ESL_DSQ *dsq, int L, const P7_OPROFILE *om)
{
  ...
  void (*fs16[MAX_BANDS + 1]) (const ESL_DSQ *, int, const P7_OPROFILE *, int, uint8_t *, uint8_t *, uint8_t **)
    = {NULL
       , calc_band_1x16,  calc_band_2x16,  calc_band_3x16,  calc_band_4x16,  calc_band_5x16,  calc_band_6x16
#if MAX_BANDS > 6
       , calc_band_7x16,  calc_band_8x16,  calc_band_9x16,  calc_band_10x16, calc_band_11x16, calc_band_12x16, calc_band_13x16, calc_band_14x16
#endif
#if MAX_BANDS > 14
       , calc_band_15, calc_band_16, calc_band_17, calc_band_18
#endif
  ...
  xEv    =  beginv;
  memset(beginv16, 128, 16);
  memset(xEv16, 128, 16);

  /* Use the highest number of bands but no more than MAX_BANDS */
  bands = (Q + MAX_BANDS - 1) / MAX_BANDS;

  uint8_t **sbv16 = (uint8_t **) malloc(om->abc->Kp * sizeof(uint8_t *));
  for (int j = 0; j < om->abc->Kp; ++j) {
	  sbv16[j] = (uint8_t *)om->sbv[j];
  }

  union { __m128i v; uint8_t c[16]; } tmpU;
  ...
  for (i = 0; i < bands; i++) {
    ...
    fs16[q-last_q](dsq, L, om, last_q, beginv16, xEv16, sbv16);

    // check
    tmpU.v = xEv;
    for(int j = 0; j < 16; ++j) {
  	  if (xEv16[j] != tmpU.c[j]) {
  		  printf("Error check @ %d xEv16\n", __LINE__);
  		  for (int k = 0; k < 16; ++k) {
  			  printf("\ti = %d xEv: %u \t xEv16: %u\n", i, tmpU.c[k], xEv16[k]);
  		  }
  		  exit(1);
  	  }
    }
  ...
  // check
  uint8_t tmp1 = esl_sse_hmax_epu8(xEv);
  uint8_t tmp2 = 0;
  for (int j = 0; j < 16; ++j) {
	  if (xEv16[j] > tmp2) tmp2 = xEv16[j];
  }
  ...
  return tmp1; //esl_sse_hmax_epu8(xEv);
}

uint8_t
get_xE_Reg(const ESL_DSQ *dsq, int L, const P7_OPROFILE *om)
{
  uint8_t xEv16[16];		/* E state: keeps max for Mk->E as we go                     */
  uint8_t beginv16[16];		/* begin scores                                              */
  ...
  uint8_t maxEv = 0;
  for (int j = 0; j < 16; ++j) {
	  if (xEv16[j] > maxEv) maxEv = xEv16[j];
  }

  if (sbv16) free(sbv16);

  return maxEv; //esl_sse_hmax_epu8(xEv);
}
