...
serial_master(ESL_GETOPTS *go, struct cfg_s *cfg)
{
  ...
  infocnt = ncpus = 1; //(ncpus == 0) ? 1 : (ncpus + 1); // one more thread is for GPU
...
//  ESL_ALLOC(info, sizeof(*info) * infocnt);
  info = (WORKER_INFO *) malloc(sizeof(*info) * infocnt);
...
pipeline_thread(void *arg)
{
  ...
  FILE *ofp = NULL;
  char str[64];
  snprintf(str, sizeof(str), "%u", threadId);
  ...
  CUDAMemPtr cudaPtr;
  memset(&cudaPtr, 0, sizeof(cudaPtr));
  ...
  {
	  allocCUDAFixed(hostPtr, cudaPtr, info->om, GPU_SIZE);
  }
