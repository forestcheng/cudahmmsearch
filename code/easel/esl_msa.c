...
{
  int   status;
  int   old, newmsa;		/* old & new allocation sizes (max # seqs) */
  int   i,j;
   ...
   */
  if (msa->aseq) ESL_REALLOC(msa->aseq, sizeof(char *)    * newmsa);
#ifdef eslAUGMENT_ALPHABET
  if (msa->ax)   ESL_REALLOC(msa->ax,   sizeof(ESL_DSQ *) * newmsa);
#endif /*eslAUGMENT_ALPHABET*/

  ESL_REALLOC(msa->sqname, sizeof(char *) * newmsa);
  ESL_REALLOC(msa->wgt,    sizeof(double) * newmsa);
  ESL_REALLOC(msa->sqlen,  sizeof(int64_t)* newmsa);

  if (msa->ss)
    {
      ESL_REALLOC(msa->ss,    sizeof(char *)  * newmsa);
      ESL_REALLOC(msa->sslen, sizeof(int64_t) * newmsa);
    }
  
  if (msa->sa)
    {
      ESL_REALLOC(msa->sa,    sizeof(char *)  * newmsa);
      ESL_REALLOC(msa->salen, sizeof(int64_t) * newmsa);
    }

  if (msa->pp)
    {
      ESL_REALLOC(msa->pp,    sizeof(char *)  * newmsa);
      ESL_REALLOC(msa->pplen, sizeof(int64_t) * newmsa);
    }

  if (msa->sqacc)   ESL_REALLOC(msa->sqacc,  sizeof(char *) * newmsa);
  if (msa->sqdesc)  ESL_REALLOC(msa->sqdesc, sizeof(char *) * newmsa);

  for (i = old; i < newmsa; i++)
    {
  ...
  if (msa->gs)
	...
	if (msa->gs[i])
	  {
	    ESL_REALLOC(msa->gs[i], sizeof(char *) * newmsa);
	    for (j = old; j < newmsa; j++)
	      msa->gs[i][j] = NULL;
	  }
      }
  ...
  if (msa->gr)
	...
	if (msa->gr[i])
	  {
	    ESL_REALLOC(msa->gr[i], sizeof(char *) * newmsa);
	    for (j = old; j < newmsa; j++)
	      msa->gr[i][j] = NULL;
	  }
      }
...
esl_msa_Copy(const ESL_MSA *msa, ESL_MSA *newmsa)
{
   ...
   * <newmsa> must have one of them allocated already.
   */
  ...
  esl_strdup(msa->mm,      -1, &(newmsa->mm));

  if (msa->sqacc != NULL) {
    ESL_ALLOC(newmsa->sqacc, sizeof(char **) * newmsa->sqalloc);
    for (i = 0; i < msa->nseq;    i++) esl_strdup(msa->sqacc[i], -1, &(newmsa->sqacc[i]));
    for (     ; i < newmsa->sqalloc; i++) newmsa->sqacc[i] = NULL;
  }
  if (msa->sqdesc != NULL) {
    ESL_ALLOC(newmsa->sqdesc, sizeof(char **) * newmsa->sqalloc);
    for (i = 0; i < msa->nseq;    i++) esl_strdup(msa->sqdesc[i], -1, &(newmsa->sqdesc[i]));
    for (     ; i < newmsa->sqalloc; i++) newmsa->sqdesc[i] = NULL;
  }
  if (msa->ss != NULL) {
    ESL_ALLOC(newmsa->ss, sizeof(char **) * newmsa->sqalloc);
    for (i = 0; i < msa->nseq;    i++) esl_strdup(msa->ss[i], -1, &(newmsa->ss[i]));
    for (     ; i < newmsa->sqalloc; i++) newmsa->ss[i] = NULL;
  }
  if (msa->sa != NULL) {
    ESL_ALLOC(newmsa->sa, sizeof(char **) * msa->nseq);
    for (i = 0; i < msa->nseq;    i++) esl_strdup(msa->sa[i], -1, &(newmsa->sa[i]));
    for (     ; i < newmsa->sqalloc; i++) newmsa->sa[i] = NULL;
  }
  if (msa->pp != NULL) {
    ESL_ALLOC(newmsa->pp, sizeof(char **) * msa->nseq);
    for (i = 0; i < msa->nseq;    i++) esl_strdup(msa->pp[i], -1, &(newmsa->pp[i]));
    for (     ; i < newmsa->sqalloc; i++) newmsa->pp[i] = NULL;
  }
  ...
  if (msa->ncomment > 0) {
    ESL_ALLOC(newmsa->comment, sizeof(char **) * msa->ncomment);
  ...
  if (msa->ngf > 0) {
    ESL_ALLOC(newmsa->gf_tag, sizeof(char **) * msa->ngf);
    ESL_ALLOC(newmsa->gf,     sizeof(char **) * msa->ngf);
    newmsa->ngf       = msa->ngf;
  ...
  if (msa->ngs > 0) {
    ESL_ALLOC(newmsa->gs_tag, sizeof(char **)  * msa->ngs);
    ESL_ALLOC(newmsa->gs,     sizeof(char ***) * msa->ngs);
    newmsa->ngs       = msa->ngs;
    for (i = 0; i < msa->ngs; i++) {
      ESL_ALLOC(newmsa->gs[i], sizeof(char **) * msa->nseq);
  ...
  if (msa->ngc > 0) {
    ESL_ALLOC(newmsa->gc_tag, sizeof(char **) * msa->ngc);
    ESL_ALLOC(newmsa->gc,     sizeof(char **) * msa->ngc);
    newmsa->ngc       = msa->ngc;
    for (i = 0; i < msa->ngc; i++) {
  ...
  if (msa->ngr > 0) {
    ESL_ALLOC(newmsa->gr_tag, sizeof(char **)  * msa->ngr);
    ESL_ALLOC(newmsa->gr,     sizeof(char ***) * msa->ngr);
    newmsa->ngr       = msa->ngr;
    for (i = 0; i < msa->ngr; i++) {
      ESL_ALLOC(newmsa->gr[i], sizeof(char **) * msa->nseq);
...
esl_msa_SequenceSubset(const ESL_MSA *msa, const int *useme, ESL_MSA **ret_new)
{
  ...
  *ret_new = NULL;
  ...
  for (nidx = 0, oidx = 0; oidx < msa->nseq; oidx++)
	...
	if (msa->sqacc  && msa->sqacc[oidx]  && (status = esl_msa_SetSeqAccession  (newmsa, nidx, msa->sqacc[oidx],  -1)) != eslOK) goto ERROR;
	if (msa->sqdesc && msa->sqdesc[oidx] && (status = esl_msa_SetSeqDescription(newmsa, nidx, msa->sqdesc[oidx], -1)) != eslOK) goto ERROR;
	if (msa->ss     && msa->ss[oidx]     && (status = msa_set_seq_ss           (newmsa, nidx, msa->ss[oidx]))         != eslOK) goto ERROR;
	if (msa->sa     && msa->sa[oidx]     && (status = msa_set_seq_sa           (newmsa, nidx, msa->sa[oidx]))         != eslOK) goto ERROR;
	if (msa->pp     && msa->pp[oidx]     && (status = msa_set_seq_pp           (newmsa, nidx, msa->pp[oidx]))         != eslOK) goto ERROR;

	/* unparsed annotation */
	for(i = 0; i < msa->ngs; i++) { if (msa->gs[i] && msa->gs[i][oidx] && (status = esl_msa_AddGS   (newmsa, msa->gs_tag[i], -1, nidx, msa->gs[i][oidx], -1)) != eslOK) goto ERROR; }
	for(i = 0; i < msa->ngr; i++) { if (msa->gr[i] && msa->gr[i][oidx] && (status = esl_msa_AppendGR(newmsa, msa->gr_tag[i],     nidx, msa->gr[i][oidx]))     != eslOK) goto ERROR; }
  ...
  if ((status = esl_strdup(msa->mm,      msa->alen, &(newmsa->mm)))      != eslOK) goto ERROR;
