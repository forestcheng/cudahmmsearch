...
#include "defs.h"
int
p7_MSVFilterReg(const ESL_DSQ *dsq, int L, const P7_OPROFILE *om, P7_OMX *ox, float *ret_sc)
{
	  ...
	  uint8_t biasv16;		/* emission bias in a vector                                 */
	  uint8_t xBv16;		/* B state: splatted vector of B[i-1] for B->Mk calculations */

	  uint8_t  xJv16;		/* special states' scores    vector for states score        */
	  int i;			   /* counter over sequence positions 1..L                      */
	  int q;			   /* counter over vectors 0..nq-1                              */
	  int Q = p7O_NQB(om->M);   /* segment length: # of vectors                              */
	  ...
	  uint8_t tjbmv16;		/* vector for cost of moving from either J or N through B to an M state */
	  uint8_t tecv16;		/* vector for E->C  cost                                     */
	  uint8_t basev16;		/* offset for scores                                         */
	...
	//  if (Q > ox->allocQ16)  ESL_EXCEPTION(eslEINVAL, "DP matrix allocated too small");
	  ...
	  /* Try highly optimized ssv filter first */
//	  int status = p7_SSVFilter(dsq, L, om, ret_sc);
//	  if (status != eslENORESULT) return status;
	  ...
	  uint8_t *dp16 = (uint8_t *) malloc((Q << 4) * sizeof(uint8_t));	/* we're going to use dp[0][0..q..Q-1], not {MDI}MX(q) macros*/
	  memset(dp16, 0, (Q << 4) * sizeof(uint8_t));

	  basev16 = om->base_b;

	  tjbmv16 = om->tjb_b + om->tbm_b;
	  ...
	  uint8_t **rbv16 = (uint8_t **) malloc(om->abc->Kp * sizeof(uint8_t *));
//	  uint8_t *rbv16mem = (uint8_t *) malloc((Q << 4) * om->abc->Kp * sizeof(uint8_t));
//
//	  memcpy(rbv16mem, om->rbv[0], (Q << 4) * om->abc->Kp *sizeof(uint8_t));
//	  uint8_t *rbv16mem = (uint8_t *)om->rbv[0];
	  for (int j = 0; j < om->abc->Kp; ++j) {
//		  rbv16[j] = rbv16mem + j * (Q << 4);
		  rbv16[j] = (uint8_t *)om->rbv[j];
	  }
...
//	    		  free(rbv16mem);
	  ...
	  /* finally C->T, and add our missing precision on the NN,CC,JJ back */
	  *ret_sc = ((float) (xJv16 - om->tjb_b) - (float) basev16);
	  *ret_sc /= om->scale_b;
	  *ret_sc -= 3.0; /* that's ~ L \log \frac{L}{L+3}, for our NN,CC,JJ */
...
int
p7_MSVFilterRegDBG(const ESL_DSQ *dsq, int L, const P7_OPROFILE *om, P7_OMX *ox, float *ret_sc)
{
  register __m128i mpv;            /* previous row values                                       */
  register __m128i xEv;		   /* E state: keeps max for Mk->E as we go                     */
  register __m128i xBv;		   /* B state: splatted vector of B[i-1] for B->Mk calculations */
  ...
  uint8_t mpv16[16], xEv16[16], sv16[16], biasv16, xBv16;
  ...
  __m128i *dp  = ox->dpb[0];	   /* we're going to use dp[0][0..q..Q-1], not {MDI}MX(q) macros*/
  uint8_t *dp16 = (uint8_t *) malloc((Q << 4) * sizeof(uint8_t));
  memset(dp16, 0, (Q << 4) * sizeof(uint8_t));
  ...
  uint8_t xJv16, tjbmv16, tecv16, basev16;
...
//  memcpy(rbv16mem, om->rbv[0], (Q << 4) * om->abc->Kp *sizeof(uint8_t));
  ...
  for (i = 1; i <= L; i++)
  {
      ...
      for(int j = 0; j < 16; ++j) {
    	  if (mpv16[j] != tmpU.c[j]) {
    		  printf("Error check @ %d mpv\n", __LINE__);
    		  for (int k = 0; k < 16; ++k) {
    			  printf("\t mpv: %u \t mpv16: %u\n", tmpU.c[k], mpv16[k]);
    		  }
    		  exit(1);
    	  }
      }
